...
1.Support increase, delete, modify, query and other operations for nodes, relationships, subgraph and graph;
2.Support batch processing;
3.Support command line;
4.Support read the data file, batch import and export for the required file format.
...
class Database():
    ...
    It support python command line parameter processing of relational database and graph database.
    You can view all the features by 'python xxx.py -h'.
    支持关系数据库和图形数据库的python命令行参数处理。可以通过'python xxx.py -h'查看所有功能。
    ...
    def __init__(self, password=None):
        ...
        if self.options.add == "Scene":
            scenes = AddScenes()
            filelist = scenes.dir_process(1, self.options.filepath, style="filelist")
    ...
    def add_qa(self, nodeclass="NluCell", name=None, content=None, topic="", behavior="", parameter="", url="", tag="", keywords="", api="", txt="", img="", chart="", delimiter=None):
        ...
        for question in questions:
            ...
            node = Node(nodeclass, name=question, content=content, topic=topic, behavior=behavior, parameter=parameter, url=url, tag=tag, keywords=keywords, api=api, txt=txt, img=img, chart=chart, hot="0")
            ...
            if data: 
                ...
                try:                                              
                    ...
                    for i in range(2, nrows):
                        ...
                        self.add_qa(name=name, content=content, topic=topic, behavior=behavior, parameter=parameter, url=url, tag=tag, keywords=keywords, api=api, txt=txt, img=img, chart=chart, delimiter="|")
