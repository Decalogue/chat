...
def scene(question="", userid="1"):
    url = "http://116.62.10.61:8080/Demo/toJson.action"
    data = {
        "robotId": userid,
        "triggerContent": question
    }
    try:
        result = json.loads(requests.get(url, data).text)
    except:
        result = ""
    return result
